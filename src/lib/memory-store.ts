...
declare global {
  var __ganttStore: {
    ...
    tasks: Map<string, GanttTask & { projectId: string }>
    initialized: boolean
  } | undefined
}
...
function getStore() {
  if (!globalThis.__ganttStore) {
    globalThis.__ganttStore = {
      projects: new Map(),
      tasks: new Map(),
      initialized: false,
    }
  }
  return globalThis.__ganttStore
}
...
function createSeedData(): void {
  ...
  projectsData.forEach((projectData) => {
    ...
    // Create tasks first to get their IDs for dependencies
    ...
    projectData.tasks.forEach((taskData) => {
      ...
      store.tasks.set(taskId, { ...task, projectId })
      tasks.push(task)
    })
